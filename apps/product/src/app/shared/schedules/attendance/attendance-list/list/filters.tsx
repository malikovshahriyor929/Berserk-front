...
export default function SessionFilters<TData extends Record<string, any>>({
  table,
...
function FilterElements<T extends Record<string, any>>({
  ...
  const sessionDate =
    table.getColumn('sessionDate')?.getFilterValue() ?? ([null, null] as any);
  const isFiltered =
  ...
  return (
    <>
      <DateFiled
        ...
        endDate={getDateRangeStateValues(sessionDate[1])!}
        selected={getDateRangeStateValues(sessionDate[0])}
        startDate={getDateRangeStateValues(sessionDate[0])!}
        onChange={(date) =>
          table.getColumn('sessionDate')?.setFilterValue(date)
        }
        inputProps={{
          label: 'Session Date',
        }}
