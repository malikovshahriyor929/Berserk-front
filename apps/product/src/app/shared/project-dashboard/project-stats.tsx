...
function StatCard({ className, transaction }: StatCardProps) {
  ...
  return (
    <Box
      className={cn(
        'group inline-block w-full rounded-lg border border-muted p-6 first:bg-mainBlue dark:bg-gray-100/50',
        className
      )}
    >
      <Flex justify="between" gap="5" className="mb-4">
        <Box className="grow space-y-2">
          <Text className="text-[22px] font-bold text-mainBlue group-first:text-gray-0 dark:text-gray-700 dark:group-first:text-gray-700 2xl:text-[20px] 3xl:text-3xl">
            {formatNumber(amount)}
          </Text>
